...
class Move
  ...
  def not_moved_yet(color, square)
    (color == :white && square < 16) ||
    (color == :black && square > 47) 
  end
 ...
 # @returns an array of moves from current position for all pieces
  ...
  def gen_all_moves(color)
    ...
    moves +=  gen_bishop_moves  color 
    moves +=  gen_rook_moves    color 
    moves +=  gen_queen_moves   color 
    moves +=  gen_king_moves    color 
    moves
  end
